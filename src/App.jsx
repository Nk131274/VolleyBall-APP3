...
const dateToISO = ymd => `20${ymd.slice(0,2)}-${ymd.slice(2,4)}-${ymd.slice(4,6)}`;
const LS = {
  get: (k, d) => { try { const v = localStorage.getItem(k); return v ? JSON.parse(v) : d; } catch { return d; } },
  set: (k, v) => { try { localStorage.setItem(k, JSON.stringify(v)); } catch {} },
};

// ─── SETUP CAMPO (for setup screen) ──────────────────────────────────────────
function CampoSetup({ rot, sp, highlight, interactive, onSelect, roster = [] }) {
  ...
  return (
    <div style={{ borderRadius: 12, overflow: "hidden", border: "1px solid #1e3a5f" }}>
      <div style={{ background: "#0d2137", textAlign: "center", fontSize: 10, color: "#4d9fdb", padding: "4px 0", letterSpacing: 2, fontWeight: 700 }}>▲  R E T E  ▲</div>
      {LAYOUT.map((row, ri) => (
        <div key={ri} style={{ display: "flex" }}>
          {row.map(pos => {
            ...
            return (
              <div key={pos} onClick={() => interactive && onSelect && onSelect(pos)}
                style={{ flex: 1, padding: "7px 2px", textAlign: "center", background: isSP ? "#1a3a1a" : isHL ? "#1a2a3a" : "#0a1929", borderRight: "1px solid #1e3a5f", borderBottom: ri === 0 ? "1px solid #1e3a5f" : "none", cursor: interactive ? "pointer" : "default" }}>
                <div style={{ fontSize: 15, fontWeight: 800, fontFamily: "monospace", color: isSP ? "#22c55e" : isHL ? "#60a5fa" : "#e2e8f0" }}>
                  {j ? `#${j}` : interactive ? pos : "—"}
                </div>
                {p && <div style={{ fontSize: 8, color: "#4d7fa8" }}>{p.name}</div>}
...
function ScoreFlash({ score, team, flash }) {
  return (
    <span style={{ display: "inline-block", fontSize: 48, fontWeight: 900, letterSpacing: -2, fontFamily: "'Courier New',monospace", color: team === "A" ? "#60a5fa" : "#fb923c", transition: "transform 0.1s", transform: flash ? "scale(1.15)" : "scale(1)", textShadow: flash ? (team === "A" ? "0 0 20px #60a5fa88" : "0 0 20px #fb923c88") : "none" }}>
      {score}
    </span>
  );
}
...
function StatsTable({ jerseys, statsMap, roster }) {
  if (!jerseys.length) return <div style={{ textAlign: "center", color: "#4d7fa8", padding: 16, fontSize: 13 }}>Nessun dato</div>;
  return (
    <div style={{ background: "#0d1f35", borderRadius: 12, overflow: "hidden", border: "1px solid #1e3a5f" }}>
      <div style={{ display: "grid", gridTemplateColumns: "1.4fr .6fr .6fr .6fr .6fr .7fr .7fr", background: "#060e1a", padding: "7px 8px", gap: 2 }}>
        {["Giocatrice","Pt","Ace","Err","Batt","%Ace","%Err"].map(h => (
          <div key={h} style={{ fontSize: 8, color: "#4d7fa8", fontWeight: 700, textAlign: "center" }}>{h}</div>
        ))}
      </div>
      {jerseys.map(j => {
        ...
        const C = ({ v, col }) => <div style={{ textAlign: "center", fontWeight: 700, fontSize: 13, color: (v && v !== "—") ? col : "#1e3a5f" }}>{v ?? 0}</div>;
        return (
          <div key={j} style={{ display: "grid", gridTemplateColumns: "1.4fr .6fr .6fr .6fr .6fr .7fr .7fr", padding: "8px", borderTop: "1px solid #1e2d3d", gap: 2 }}>
            <div><div style={{ fontFamily: "monospace", fontWeight: 900, fontSize: 12, color: "#e2e8f0" }}>#{j}</div><div style={{ fontSize: 8, color: "#4d7fa8" }}>{p.name}</div></div>
...
// ─── POSITION CARD (game screen) ─────────────────────────────────────────────
function PositionCard({ pos, rot, sp, roster, liberoJ, liberoOn, serve,
  onAttack, onError, onAce, onServeError, onGenericError,
  onTap, tapMode, selected, isCorrectSrc }) {
  const jersey  = rot[pos - 1];
  const isP1    = pos === 1;
  const isLib   = liberoOn && jersey === liberoJ;
  const p       = roster.find(r => r.jersey === jersey) || { name: "", role: "" };
  const aServes = serve === "A";
  const canServeButtons = isP1 && aServes && !isLib;

  const stop = fn => e => { e.stopPropagation(); fn(); };

  const abtn = (bg, label, disabled) => ({
    style: {
      background: disabled ? "#0d0d0d" : bg, border: "none", borderRadius: 6,
      color: disabled ? "#2a2a2a" : "#fff", fontWeight: 700, fontSize: 10,
      padding: "5px 2px", cursor: disabled ? "not-allowed" : "pointer",
      opacity: disabled ? 0.35 : 1, flex: 1, lineHeight: 1.25, textAlign: "center",
    },
  });

  const highlight = selected ? "#1a3a5a" : isCorrectSrc ? "#2a1a3a" : isP1 ? "#0e1e3a" : "#0a1422";
  const border    = selected ? "#60a5fa"  : isCorrectSrc ? "#a855f7"  : isP1 && aServes ? "#2563eb66" : "#1e3a5f";

  return (
    <div onClick={tapMode ? () => onTap(pos) : undefined}
      style={{ background: highlight, border: `2px solid ${border}`, borderRadius: 10,
        display: "flex", flexDirection: "column", gap: 3, padding: "4px 3px",
        cursor: tapMode ? "pointer" : "default", position: "relative", overflow: "hidden",
        transition: "border-color 0.2s, background 0.2s" }}>

      {/* Pos label + ball */}
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", opacity: 0.5 }}>
        <div style={{ fontSize: 8, color: "#4d7fa8", fontWeight: 700 }}>P{pos}</div>
        {canServeButtons && <div style={{ fontSize: 9, animation: "pulse 1.2s infinite" }}>🏐</div>}
      </div>

      {/* Player info */}
      <div style={{ textAlign: "center" }}>
        <div style={{ fontSize: isP1 ? 17 : 14, fontWeight: 900, fontFamily: "monospace", color: isLib ? "#f59e0b" : "#e2e8f0" }}>
          {isLib ? "L" : `#${jersey}`}
        </div>
        <div style={{ fontSize: 8, color: "#4d7fa8", lineHeight: 1.1, overflow: "hidden", whiteSpace: "nowrap", textOverflow: "ellipsis" }}>
          {p.name}
        </div>
      </div>
      ...
      {!tapMode && (
        <>
          {/* P1 serve-specific buttons */}
          {canServeButtons && (
            <div style={{ display: "flex", gap: 3 }}>
              <button onClick={stop(onAce)} {...abtn("linear-gradient(135deg,#15803d,#14532d)", "🏐 Ace")}>
                🏐 Ace
              </button>
              <button onClick={stop(onServeError)} {...abtn("linear-gradient(135deg,#991b1b,#7f1d1d)", "💥 Err.Batt")}>
                💥 E.Batt
              </button>
            </div>
          )}
          ...
          {/* Attack + Error */}
          <div style={{ display: "flex", gap: 3 }}>
            <button onClick={stop(onAttack)} {...abtn("linear-gradient(135deg,#1d4ed8,#1e40af)")}>
              ⚡ Punto
            </button>
            <button onClick={stop(onError)} {...abtn("linear-gradient(135deg,#c2410c,#9a3412)")}>
              ❌ Err.
            </button>
          </div>

          {/* Generic error — P1 only */}
          {isP1 && (
            <button onClick={stop(onGenericError)} style={{
              background: "#111827", border: "1px solid #374151", borderRadius: 6,
              color: "#6b7280", fontWeight: 700, fontSize: 9, padding: "4px 2px",
      ...
      {/* Tap mode overlay */}
      {tapMode && (
        <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", borderRadius: 8, background: selected ? "#1a3a5a88" : "transparent" }}>
          {selected && <div style={{ fontSize: 20 }}>✓</div>}
        </div>
      )}
    </div>
  );
...
export default function VolleyballApp() {
  ...
  // ── Team edit form
  const [rJersey, setRJersey] = useState(""); const [rName, setRName] = useState("");
  const [rRole, setRRole]     = useState(ROSTER_ROLES[0]); const [rErr, setRErr] = useState("");
  const [saveTeamModal, setSaveTeamModal] = useState(false); const [newTeamName, setNewTeamName] = useState("");
  ...
  const flashT = useRef(null); const rotT = useRef(null);
  ...
  // ── Cambio (new flow: tap position → bench picker)
  ...
  useEffect(() => { if (rJRef.current && (fase === "teamEdit")) setTimeout(() => rJRef.current?.focus(), 150); }, [fase]);
  ...
  const availablePlayers = activeRoster.filter(p => !setupJerseys.includes(p.jersey) && p.jersey !== liberoJersey);

  const buildPreviewRot = () => {
    const r = new Array(6).fill("");
    posOrdinati.forEach((p, i) => { if (i < setupJerseys.length) r[p - 1] = setupJerseys[i]; });
    return r;
  };
  ...
  const handleSetupBack = () => {
    if (setupStep === "pickPlayers") {
      if (!setupJerseys.length) { setSetupStep("setter"); setSetupSP(null); }
      else setSetupJerseys(setupJerseys.slice(0, -1));
  ...
  // Track serve count for current P1 (only when A is serving)
  const trackServe = (currentRot, currentServe, currentSetN) => {
    if (currentServe !== "A") return;
    const server = getServer(currentRot);
    setServeCounts(prev => ({ ...prev, [server]: (prev[server] || 0) + 1 }));
    setServeCountsBySet(prev => ({
      ...prev, [currentSetN]: { ...(prev[currentSetN] || {}), [server]: ((prev[currentSetN] || {})[server] || 0) + 1 }
    }));
  };

  // Central point application
  const applyPoint = (teamPoint, tipo, maglia, serveError) => {
    ...
    if (liberoJersey) {
      if (nsrv === "B" && serve === "A") {
        ...
        if (isCentrale(nr[0]) && libOn) {
          const li = nr.indexOf(liberoJersey); if (li !== -1) nr[li] = libRep;
          libOn = false; libRep = null;
        }
      }
    }
    ...
    if (checkWin(ns.A, ns.B, setN)) {
      ...
      setSetVinti(nSV); setFase(nSV[winner] >= 3 ? "matchOver" : "setOver");
    }
  };

  // Point handlers — all start by tracking serve
  const doPoint = (teamPoint, tipo, maglia) => {
    trackServe(rot, serve, setN);
    applyPoint(teamPoint, tipo, maglia);
  };
  ...
  const handleSubTapPos = pos => {
    setSubSrcPos(pos); setSubBenchOpen(true); setSubErr2("");
  };

  const handleSubPickBench = jersey => {
    if (rot.includes(jersey)) { setSubErr2(`#${jersey} è già in campo!`); return; }
    const pos = subSrcPos;
    const outJ = rot[pos - 1];
    const newRot = [...rot]; newRot[pos - 1] = jersey;
    setRot(newRot);
    setSubs(prev => [...prev, { set: setN, scoreA: score.A, scoreB: score.B, out: outJ, in: jersey, position: pos, ruolo: getRuoloS(pos, sp) }]);
  ...
  const handleCorrectTapPos = pos => {
    if (!correctSrcPos) {
      setCorrectSrcPos(pos);
    } else {
      if (pos !== correctSrcPos) {
        // Swap
        const newRot = [...rot];
        const tmp = newRot[correctSrcPos - 1];
        newRot[correctSrcPos - 1] = newRot[pos - 1];
        newRot[pos - 1] = tmp;
        setRot(newRot);
      }
  ...
  const computeStats = setNum => {
    ...
    const stats = {};
    const ensure = j => { if (!stats[j]) stats[j] = { punti: 0, ace: 0, attacchi: 0, errori: 0, erroriServ: 0, battute: 0 }; };
    rot.forEach(j => j && ensure(j));
    setLineups.forEach(sl2 => sl2.rot.forEach(j => j && ensure(j)));
    subs.forEach(s => { ensure(s.in); ensure(s.out); });
    Object.entries(sc).forEach(([j, cnt]) => { ensure(j); stats[j].battute = cnt; });
    sl.forEach(e => {
      const j = e["Maglia responsabile"]; if (!j) return; ensure(j);
      if (e["Squadra punto"] === "A") { stats[j].punti++; if (e["Tipo azione"] === "Ace") stats[j].ace++; if (e["Tipo azione"] === "Attacco vincente") stats[j].attacchi++; }
      else if (e["Tipo azione"] === "Errore in battuta") { stats[j].errori++; stats[j].erroriServ++; }
      else if (e["Tipo azione"] === "Errore") { stats[j].errori++; }
    });
    return stats;
  };
  ...
  const handleNewMatch = () => {
    ...
    setLiberoOnField(false); setLiberoReplacing(null); setOpponentName(""); setMatchDate(todayStr()); setHomeAway("home");
  };
  ...
  const exportMatchToExcel = m => {
    ...
    const uniqueSets = [...new Set(ml.map(l => l.Set))].sort((a, b) => a - b);
    const aoa1 = [[`${matchLabel(m)}`], [`Data: ${dateToISO(m.date)}  |  ${m.homeAway === "home" ? "Casa" : "Trasferta"}`], []];
    uniqueSets.forEach(sn => {
      aoa1.push([`◆ SET ${sn}`, "", "", "", "", "", "", ""]);
    ...
    XLSX.utils.book_append_sheet(wb, ws1, "Andamento Partita");
    // Stats sheet
    const allJ = [...new Set([...msl.flatMap(sl => sl.rot), ...ms.flatMap(s => [s.in, s.out])])].filter(Boolean).sort((a, b) => parseInt(a) - parseInt(b));
    const hdr = ["Maglia", "Nome", "Ruolo", ...uniqueSets.flatMap(s => [`S${s} Pt`, `S${s} Ace`, `S${s} Err`, `S${s} Batt`, `S${s} %Ace`, `S${s} %Err`]), "Tot Pt", "Tot Ace", "Tot Err", "Tot Batt", "Tot %Ace", "Tot %Err"];
    const rows = allJ.map(j => {
      const pl = mr.find(r => r.jersey === j) || { name: "", role: "" };
      const row = [`#${j}`, pl.name, pl.role];
      let tp = 0, ta = 0, te = 0, tb = 0;
      uniqueSets.forEach(sn => {
        const batt = (mscbs[sn] || {})[j] || 0;
        let pt = 0, ace = 0, err = 0;
        ml.filter(l => l.Set === sn).forEach(e => {
          if (e["Maglia responsabile"] !== j) return;
          if (e["Squadra punto"] === "A") { pt++; if (e["Tipo azione"] === "Ace") ace++; }
          else if (e["Tipo azione"] === "Errore in battuta" || e["Tipo azione"] === "Errore") err++;
        });
  ...
  const handlePosTap = pos => {
    if (subMode) handleSubTapPos(pos);
    else if (correctMode) handleCorrectTapPos(pos);
  };
    ...
    <div style={{ ...S.root, display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ ...S.card, width: "100%", maxWidth: 320, animation: "fadeIn 0.4s ease" }}>
        <div style={{ textAlign: "center", marginBottom: 22 }}>
          ...
          <div style={{ fontSize: 12, color: "#4d7fa8", marginTop: 4 }}>Accedi per continuare</div>
        </div>
  ...
  return (
    <div style={S.root}>
      <style>{`
        ...
        @keyframes rotGlow{0%{box-shadow:0 0 0 #60a5fa}50%{box-shadow:0 0 18px #60a5fa88}100%{box-shadow:0 0 0 #60a5fa}}
        @keyframes slideUp{from{transform:translateY(100%);opacity:0}to{transform:translateY(0);opacity:1}}
        @keyframes ballBounce{0%{transform:scale(1)}40%{transform:scale(1.5)}70%{transform:scale(0.85)}100%{transform:scale(1)}}
      `}</style>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100dvh", padding: 20 }}>
          <div style={{ width: "100%", maxWidth: 340 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <button onClick={() => setFase("results")} style={{ ...S.card, border: "1px solid #1e3a5f", cursor: "pointer", textAlign: "center", padding: 20 }}>
                <div style={{ fontSize: 28, marginBottom: 5 }}>📊</div>
                <div style={{ fontSize: 16, fontWeight: 900 }}>Risultati precedenti</div>
                <div style={{ fontSize: 12, color: "#4d7fa8", marginTop: 3 }}>{savedMatches.length} partite salvate</div>
              </button>
              <button onClick={() => setFase(teams.length > 0 ? "teamSelect" : "teamEdit")} style={{ ...S.card, border: "1px solid #16a34a44", cursor: "pointer", textAlign: "center", padding: 20, background: "#0d2218" }}>
                <div style={{ fontSize: 28, marginBottom: 5 }}>▶️</div>
                <div style={{ fontSize: 16, fontWeight: 900, color: "#22c55e" }}>Gioca una partita</div>
              </button>
            </div>
            <button onClick={handleLogout} style={{ width: "100%", marginTop: 18, padding: 10, background: "none", border: "none", color: "#2d5a7a", cursor: "pointer", fontSize: 12 }}>Esci ↗</button>
          </div>
        </div>
      )}
      ...
      {fase === "teamSelect" && (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: 16, minHeight: "100dvh" }}>
          <div style={{ width: "100%", maxWidth: 380, paddingTop: 16 }}>
            <div style={{ textAlign: "center", marginBottom: 18 }}>
              <div style={{ fontSize: 20, fontWeight: 900, color: "#60a5fa" }}>Seleziona squadra</div>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 12 }}>
              {teams.map(t => (
                <button key={t.id} onClick={() => handleSelectTeam(t)} style={{ ...S.card, cursor: "pointer", display: "flex", alignItems: "center", gap: 12, padding: "14px 16px" }}>
                  <div style={{ fontSize: 22 }}>👕</div>
                  <div style={{ flex: 1, textAlign: "left" }}>
                    <div style={{ fontWeight: 900, fontSize: 15 }}>{t.name}</div>
                    <div style={{ fontSize: 11, color: "#4d7fa8", marginTop: 2 }}>{t.roster.length} giocatrici</div>
                  </div>
      ...
      {fase === "teamEdit" && (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: 16, paddingBottom: 32, minHeight: "100dvh" }}>
          <div style={{ width: "100%", maxWidth: 400, paddingTop: 14 }}>
            <div style={{ textAlign: "center", marginBottom: 14 }}>
              <div style={{ fontSize: 19, fontWeight: 900, color: "#60a5fa" }}>{currentTeam ? `Modifica: ${currentTeam.name}` : "Nuova squadra"}</div>
            </div>
            <div style={{ ...S.card, marginBottom: 12 }}>
              <div style={{ ...S.label, marginBottom: 8 }}>Aggiungi giocatrice</div>
              <div style={{ display: "flex", gap: 6, marginBottom: 7 }}>
                <input ref={rJRef} type="number" inputMode="numeric" value={rJersey} onChange={e => { setRJersey(e.target.value); setRErr(""); }} onKeyDown={e => e.key === "Enter" && document.getElementById("rNI")?.focus()} placeholder="N°" style={{ ...S.inp, width: 68, textAlign: "center", padding: "10px 5px", flex: "0 0 68px" }} />
              ...
              <select value={rRole} onChange={e => setRRole(e.target.value)} style={{ ...S.inp, fontSize: 12, marginBottom: 7, appearance: "none" }}>
                {ROSTER_ROLES.map(r => <option key={r}>{r}</option>)}
              </select>
              {rErr && <div style={{ color: "#ef4444", fontSize: 11, marginBottom: 6, textAlign: "center" }}>{rErr}</div>}
              <button onClick={handleAddPlayer} style={S.btn("#1e3a5f", "#93c5fd")}>＋ Aggiungi</button>
            </div>
            {editRoster.length > 0 && (
              <div style={{ ...S.card, marginBottom: 12 }}>
                <div style={{ ...S.label, marginBottom: 8 }}>Distinta ({editRoster.length})</div>
                <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                  {editRoster.map(p => (
                    <div key={p.jersey} style={{ display: "flex", alignItems: "center", gap: 7, background: "#060e1a", borderRadius: 9, padding: "7px 10px", border: "1px solid #1e3a5f" }}>
                      <div style={{ fontFamily: "monospace", fontWeight: 900, fontSize: 15, color: p.role === "Libero" ? "#f59e0b" : "#60a5fa", width: 34 }}>#{p.jersey}</div>
                      <div style={{ flex: 1 }}>
      ...
      {fase === "matchMeta" && (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100dvh", padding: 16 }}>
          <div style={{ width: "100%", maxWidth: 360 }}>
            <div style={{ textAlign: "center", marginBottom: 18 }}>
              <div style={{ fontSize: 19, fontWeight: 900, color: "#60a5fa" }}>{currentTeam?.name}</div>
            ...
            <div style={S.card}>
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
                {[["home", "🏠 Casa"], ["away", "✈️ Trasferta"]].map(([v, l]) => (
                  <button key={v} onClick={() => setHomeAway(v)} style={{ flex: 1, padding: "11px 6px", borderRadius: 11, fontWeight: 700, fontSize: 13, border: "none", cursor: "pointer", background: homeAway === v ? "#1d4ed8" : "#1e3a5f", color: homeAway === v ? "#fff" : "#93c5fd" }}>{l}</button>
                ))}
              </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100dvh", padding: 14 }}>
          <div style={{ width: "100%", maxWidth: 390 }}>
            <div style={{ textAlign: "center", marginBottom: 12 }}>
              <div style={{ fontSize: 19, fontWeight: 900 }}>Set <span style={{ color: "#60a5fa" }}>{setN}</span></div>
              ...
              {setupStep === "setter" && (
                <>
                  <div style={{ textAlign: "center", marginBottom: 10 }}>
                    ...
                    <div style={{ fontSize: 14, fontWeight: 700, marginTop: 5, color: "#93c5fd" }}>Dove si trova il <span style={{ color: "#22c55e" }}>Palleggiatore</span>?</div>
                  </div>
                  ...
                  {liberoJersey && <div style={{ marginTop: 8, padding: "5px 10px", background: "#1a1000", borderRadius: 8, textAlign: "center", fontSize: 11 }}>
                    🟡 Libero <span style={{ color: "#f59e0b", fontFamily: "monospace", fontWeight: 700 }}>#{liberoJersey}</span> — entra automaticamente
                  </div>}
                </>
              )}
              {setupStep === "pickPlayers" && (
                <>
                  <div style={{ textAlign: "center", marginBottom: 8 }}>
                    ...
                    <div style={{ fontSize: 14, fontWeight: 700, marginTop: 4 }}>Chi è in pos.<span style={{ color: "#60a5fa" }}> {curPos}</span>? <span style={{ fontSize: 11, color: "#fbbf24" }}>({getRuoloS(curPos, setupSP)})</span></div>
                  </div>
                  ...
                  <div style={{ marginBottom: 10 }}>
                    <CampoSetup rot={buildPreviewRot()} sp={setupSP} highlight={curPos} roster={activeRoster} />
                  </div>
                  {/* Player grid — show number, name, role */}
                  <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 8 }}>
                    {availablePlayers.map(p => (
                      <button key={p.jersey} onClick={() => handlePickSetupPlayer(p.jersey)} style={{
                        display: "flex", alignItems: "center", gap: 12, padding: "10px 14px",
                        background: "#060e1a", border: "1px solid #1e3a5f", borderRadius: 12,
                        cursor: "pointer", textAlign: "left",
                      }}>
                        <div style={{ fontFamily: "monospace", fontWeight: 900, fontSize: 20, color: "#60a5fa", width: 44 }}>#{p.jersey}</div>
                        <div style={{ flex: 1 }}>
                    ...
                    {!availablePlayers.length && <div style={{ textAlign: "center", color: "#4d7fa8", fontSize: 12, padding: 10 }}>Nessuna disponibile</div>}
                  </div>
              ...
              {setupStep === "serve" && (
                <>
                  <div style={{ textAlign: "center", marginBottom: 10 }}>
                    ...
                    <div style={{ fontSize: 14, fontWeight: 700, marginTop: 5, color: "#93c5fd" }}>Chi batte per primo?</div>
                  </div>
                  <CampoSetup rot={buildPreviewRot()} sp={setupSP} roster={activeRoster} />
                  <div style={{ display: "flex", gap: 8, margin: "12px 0" }}>
                    {["A", "B"].map(sq => (
                      <button key={sq} onClick={() => setSetupServe(sq)} style={{ flex: 1, padding: "11px 6px", borderRadius: 11, fontWeight: 700, fontSize: 13, textAlign: "center", cursor: "pointer", border: "none", background: sq === "A" ? (setupServe === sq ? "#1d4ed8" : "#1e3a5f") : (setupServe === sq ? "#c2410c" : "#3a1f0d"), color: setupServe === sq ? "#fff" : sq === "A" ? "#93c5fd" : "#fdba74" }}>
                        <div style={{ fontSize: 18, marginBottom: 2 }}>🏐</div>{sq === "A" ? teamALabel : teamBLabel}
                      </button>
                    ))}
                  </div>
                  <button onClick={handleStartSet} style={S.btn("linear-gradient(135deg,#16a34a,#15803d)")}>▶  START SET {setN}</button>
                  <button onClick={handleSetupBack} style={{ width: "100%", marginTop: 7, padding: 9, background: "none", border: "none", color: "#4d7fa8", cursor: "pointer", fontSize: 12 }}>← Modifica</button>
                </>
              )}
            </div>
          </div>
      ...
      {fase === "gioco" && (
        <div style={{ display: "flex", flexDirection: "column", height: "100dvh" }}>
          ...
          {/* HEADER */}
          <div style={{ background: "#0a1929", borderBottom: "1px solid #1e3a5f", padding: "6px 12px", flexShrink: 0 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 3 }}>
              <div style={{ fontSize: 10, color: "#4d7fa8", fontWeight: 700 }}>SET {setN}</div>
              <div style={{ display: "flex", gap: 7, alignItems: "center" }}>
                <span style={S.tag("#60a5fa")}>A: {setVinti.A}</span>
                <span style={{ color: "#2d5a7a", fontSize: 9 }}>set</span>
                <span style={S.tag("#fb923c")}>B: {setVinti.B}</span>
              </div>
              <div style={{ fontSize: 10, fontWeight: 700, color: serve === "A" ? "#60a5fa" : "#fb923c", padding: "3px 7px", borderRadius: 7, background: serve === "A" ? "#1e3a5f44" : "#3a1f0d44" }}>
                {serve === "A" ? <span>🏐 <span style={{ fontFamily: "monospace" }}>#{getServer(rot)}</span></span> : <span>Batte {teamBLabel}</span>}
              </div>
            </div>
            {/* Ball + Score */}
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0 4px" }}>
              <div style={{ fontSize: 16, opacity: serve === "A" ? 1 : 0.15, transition: "all 0.4s", filter: serve === "A" ? "drop-shadow(0 0 5px #60a5fa)" : "none" }}>🏐</div>
              ...
              <div style={{ fontSize: 16, opacity: serve === "B" ? 1 : 0.15, transition: "all 0.4s", filter: serve === "B" ? "drop-shadow(0 0 5px #fb923c)" : "none" }}>🏐</div>
            </div>
          </div>
          ...
          <div style={{
            ...
            gap: 4, padding: 4, flex: 1, minHeight: 0,
            animation: rotAnim ? "rotGlow 0.6s ease" : "none",
          }}>
            {[...FRONT_ROW, ...BACK_ROW].map(pos => (
              <PositionCard key={pos} pos={pos} rot={rot} sp={sp} roster={activeRoster}
                liberoJ={liberoJersey} liberoOn={liberoOnField} serve={serve}
                onAttack={() => handlePlayerAttack(rot[pos - 1])}
                onError={() => handlePlayerError(rot[pos - 1])}
                onAce={handleAce}
                onServeError={handleServeError}
                onGenericError={handleGenericError}
                onTap={tapMode ? handlePosTap : undefined}
                tapMode={!!tapMode}
          ...
          {/* GUEST ACTIONS */}
          <div style={{ display: "flex", gap: 4, padding: "4px 4px", background: "#0a0800", borderTop: "1px solid #3a1f0d55", borderBottom: "1px solid #1e3a5f44", flexShrink: 0 }}>
            <div style={{ fontSize: 9, color: "#f97316", fontWeight: 700, alignSelf: "center", flexShrink: 0, marginRight: 2 }}>
              {teamBLabel.slice(0, 8).toUpperCase()}
            </div>
            {[
              ...
              { label: "❌ Errore → Pt A", fn: handleGuestError, dis: false, bg: "#1a1a1a" },
            ].map(({ label, fn, dis, bg }) => (
              <button key={label} onClick={dis ? undefined : fn} style={{
                flex: 1, padding: "7px 3px", borderRadius: 9, fontWeight: 700, fontSize: 10,
          ...
          {/* BOTTOM BAR */}
          <div style={{ background: "#0a1929", borderTop: "1px solid #1e3a5f", padding: "6px 7px", flexShrink: 0 }}>
            {/* Mode indicator */}
            {(subMode || correctMode) && (
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: subMode ? "#1a1500" : "#1a003a", border: `1px solid ${subMode ? "#ca8a0444" : "#7c3aed44"}`, borderRadius: 8, padding: "5px 10px", marginBottom: 6 }}>
                <div style={{ fontSize: 11, color: subMode ? "#ca8a04" : "#a855f7", fontWeight: 700 }}>
                  {subMode ? (subSrcPos ? `Esce: #${rot[subSrcPos - 1]} — seleziona chi entra` : "🔄 Tocca il posto di chi esce") : (correctSrcPos ? `P${correctSrcPos} selezionata — tocca destinazione` : "🔧 Tocca la posizione da correggere")}
                </div>
            ...
            <div style={{ display: "flex", gap: 5 }}>
              {[
                { label: "🔄 Cambio", fn: () => { setSubMode(true); setSubSrcPos(null); setCorrectMode(false); setCorrectSrcPos(null); }, active: subMode, col: "#ca8a04" },
                { label: "📊 Stats",  fn: () => { setStatsTab("current"); setShowStats(true); }, col: "#93c5fd" },
                { label: "🔧 Correggi", fn: () => { setCorrectMode(true); setCorrectSrcPos(null); setSubMode(false); setSubSrcPos(null); }, active: correctMode, col: "#a855f7" },
                prevSnap ? { label: "↩", fn: handleUndo, col: "#22c55e" } : null,
                { label: "📥", fn: handleExport, col: "#4d7fa8" },
              ].filter(Boolean).map(({ label, fn, active, col }) => (
                <button key={label} onClick={fn} style={{
                  flex: label === "↩" || label === "📥" ? "0 0 auto" : 1,
                  padding: "8px 4px", borderRadius: 9,
                  background: active ? col + "22" : "#0d1f35",
                  border: `1px solid ${active ? col + "88" : "#1e3a5f"}`,
                  color: col, fontWeight: 700, fontSize: 11, cursor: "pointer",
                }}>{label}</button>
              ))}
            </div>
          </div>
      ...
      {fase === "setOver" && (
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: "100dvh", padding: 18 }}>
          <div style={{ ...S.card, textAlign: "center", maxWidth: 320, width: "100%" }}>
            <div style={{ fontSize: 40, marginBottom: 6 }}>🏆</div>
            <div style={{ ...S.label, marginBottom: 5 }}>Fine Set {setN}</div>
            <div style={{ fontSize: 19, fontWeight: 900, color: endWinner === "A" ? "#60a5fa" : "#fb923c", marginBottom: 10 }}>Vince {endWinner === "A" ? teamALabel : teamBLabel}!</div>
            <div style={{ fontSize: 38, fontWeight: 900, marginBottom: 3, fontFamily: "monospace" }}>
              <span style={{ color: "#60a5fa" }}>{score.A}</span><span style={{ color: "#1e3a5f", margin: "0 6px" }}>:</span><span style={{ color: "#fb923c" }}>{score.B}</span>
            </div>
            <div style={{ display: "flex", justifyContent: "center", gap: 12, marginTop: 5, marginBottom: 14 }}>
            ...
            <button onClick={handleExport} style={{ width: "100%", marginTop: 8, padding: 9, borderRadius: 11, background: "#0d1f35", border: "1px solid #1e3a5f", color: "#4d7fa8", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>📥 Excel</button>
          </div>
        </div>
      )}
      ...
      {/* ═══ MATCH OVER ═══ */}
      {fase === "matchOver" && (() => {
        const [saved, setSaved] = useState(false); const [savedRec, setSavedRec] = useState(null);
        return (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: "100dvh", padding: 18 }}>
            <div style={{ ...S.card, textAlign: "center", maxWidth: 330, width: "100%" }}>
              <div style={{ fontSize: 40, marginBottom: 6 }}>🏆🎉</div>
              <div style={{ ...S.label, marginBottom: 5 }}>Partita Terminata</div>
              <div style={{ fontSize: 20, fontWeight: 900, marginBottom: 8, color: endWinner === "A" ? "#60a5fa" : "#fb923c" }}>Vince {endWinner === "A" ? teamALabel : teamBLabel}!</div>
              <div style={{ background: "#060e1a", borderRadius: 12, padding: "12px 16px", margin: "9px 0 14px", border: "1px solid #1e3a5f" }}>
                ...
                <div style={{ fontSize: 34, fontWeight: 900, fontFamily: "monospace" }}>
                  <span style={{ color: "#60a5fa" }}>{setVinti.A}</span><span style={{ color: "#1e3a5f", margin: "0 6px" }}>:</span><span style={{ color: "#fb923c" }}>{setVinti.B}</span>
                </div>
              </div>
              {!saved
                ? <button onClick={() => { const r = buildMatchRecord(); setSavedMatches(prev => [...prev, r]); setSaved(true); setSavedRec(r); }} style={S.btn("linear-gradient(135deg,#7c3aed,#6d28d9)")}>💾 Salva partita</button>
                : <div style={{ background: "#1a3a1a", borderRadius: 10, padding: 10, marginBottom: 4, border: "1px solid #22c55e44" }}>
                  <div style={{ fontSize: 12, color: "#22c55e", fontWeight: 700 }}>✓ Partita salvata</div>
                  <div style={{ fontSize: 10, color: "#4d7fa8", marginTop: 2 }}>{savedRec && matchLabel(savedRec)}</div>
                </div>
              }
              <button onClick={handleExport} style={{ width: "100%", marginTop: 8, padding: 10, borderRadius: 12, background: "#0d1f35", border: "1px solid #1e3a5f", color: "#4d7fa8", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>📥 Excel</button>
              <button onClick={handleNewMatch} style={{ width: "100%", marginTop: 8, padding: 9, borderRadius: 11, background: "none", border: "1px solid #1e3a5f", color: "#4d7fa8", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>🏠 Home</button>
            </div>
          </div>
        );
      })()}
      ...
      {fase === "results" && (
        <div style={{ display: "flex", flexDirection: "column", minHeight: "100dvh" }}>
          <div style={{ background: "#0a1929", borderBottom: "1px solid #1e3a5f", padding: "14px 16px", display: "flex", alignItems: "center", gap: 12 }}>
            <button onClick={() => setFase("home")} style={{ background: "none", border: "none", color: "#4d7fa8", fontSize: 20, cursor: "pointer" }}>←</button>
            <div style={{ fontWeight: 900, fontSize: 16 }}>Risultati precedenti</div>
          </div>
          <div style={{ flex: 1, overflowY: "auto", padding: 14 }}>
            {!savedMatches.length && <div style={{ textAlign: "center", color: "#4d7fa8", padding: 40 }}><div style={{ fontSize: 38, marginBottom: 10 }}>📋</div>Nessuna partita salvata</div>}
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {[...savedMatches].reverse().map(m => {
                ...
                return (
                  <div key={m.id} style={{ background: "#0d1f35", borderRadius: 12, border: `1px solid ${weWon ? "#16a34a44" : "#7f1d1d44"}`, padding: "11px 13px", display: "flex", alignItems: "center", gap: 10 }}>
                    <div style={{ flex: 1, cursor: "pointer" }} onClick={() => { setDetailMatch(m); setDetailTab("overview"); setFase("matchDetail"); }}>
                      <div style={{ fontWeight: weWon ? 900 : 500, fontSize: 13, color: weWon ? "#22c55e" : "#9ca3af", lineHeight: 1.4 }}>{matchLabel(m)}</div>
                      <div style={{ fontSize: 10, color: "#4d7fa8", marginTop: 2 }}>{m.teamName} • {m.homeAway === "home" ? "Casa" : "Trasferta"}</div>
                    </div>
                    <button onClick={() => setSavedMatches(prev => prev.filter(x => x.id !== m.id))} style={{ background: "none", border: "1px solid #3a1f0d", borderRadius: 8, padding: "5px 9px", color: "#ef4444", cursor: "pointer", fontSize: 12 }}>✕</button>
                  </div>
                );
              })}
            </div>
      ...
      {fase === "matchDetail" && detailMatch && (() => {
        ...
        const uniqueSets = [...new Set(m.log.map(l => l.Set))].sort((a, b) => a - b);
        const tabs = [{ key: "overview", label: "📋" }, ...uniqueSets.map(s => ({ key: `s${s}`, label: `S${s}` })), { key: "total", label: "Tot" }];
        const h = m.homeAway === "home"; const weWon = (h ? m.setsA : m.setsB) > (h ? m.setsB : m.setsA);

        const getDetailStats = tab => {
        ...
        return (
          <div style={{ display: "flex", flexDirection: "column", minHeight: "100dvh" }}>
            <div style={{ background: "#0a1929", borderBottom: "1px solid #1e3a5f", padding: "12px 14px", display: "flex", alignItems: "center", gap: 10 }}>
              <button onClick={() => setFase("results")} style={{ background: "none", border: "none", color: "#4d7fa8", fontSize: 20, cursor: "pointer" }}>←</button>
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 900, fontSize: 12, color: weWon ? "#22c55e" : "#ef4444" }}>{matchLabel(m)}</div>
              </div>
              <button onClick={() => exportMatchToExcel(m)} style={{ padding: "6px 10px", borderRadius: 9, background: "#0d1f35", border: "1px solid #1e3a5f", color: "#4d7fa8", fontWeight: 700, fontSize: 11, cursor: "pointer" }}>📥</button>
            </div>
            <div style={{ display: "flex", gap: 5, padding: "8px 12px", overflowX: "auto", background: "#0a1929", borderBottom: "1px solid #1e3a5f" }}>
              {tabs.map(t => <button key={t.key} onClick={() => setDetailTab(t.key)} style={{ padding: "5px 11px", borderRadius: 20, fontWeight: 700, fontSize: 10, cursor: "pointer", whiteSpace: "nowrap", border: `1px solid ${detailTab === t.key ? "#3b82f6" : "#1e3a5f"}`, flexShrink: 0, background: detailTab === t.key ? "#1d4ed8" : "#0d1f35", color: detailTab === t.key ? "#fff" : "#4d7fa8" }}>{t.label}</button>)}
            </div>
            <div style={{ flex: 1, overflowY: "auto", padding: 14 }}>
              {detailTab === "overview" && (
                <div>
                  {uniqueSets.map(sn => {
                    const last = m.log.filter(l => l.Set === sn).slice(-1)[0];
                    const aw = last && last["Punti A"] > last["Punti B"];
                    return <div key={sn} style={{ background: "#0d1f35", borderRadius: 12, padding: "12px 14px", marginBottom: 8, border: "1px solid #1e3a5f", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <div style={{ fontSize: 12, color: "#4d7fa8" }}>Set {sn}</div>
                      <div style={{ fontFamily: "monospace", fontWeight: 900, fontSize: 22 }}>
                        <span style={{ color: aw ? "#22c55e" : "#60a5fa" }}>{last?.["Punti A"] ?? 0}</span>
                        <span style={{ color: "#1e3a5f", margin: "0 5px" }}>:</span>
                        <span style={{ color: !aw ? "#22c55e" : "#fb923c" }}>{last?.["Punti B"] ?? 0}</span>
                      </div>
                      <div style={{ fontSize: 11, color: aw ? "#22c55e" : "#ef4444", fontWeight: 700 }}>{aw ? m.teamName : m.opponentName}</div>
                    </div>;
                  })}
                </div>
              )}
              {detailTab !== "overview" && (() => {
                const st = getDetailStats(detailTab);
                const js = mr.map(p => p.jersey).filter(j => st[j]);
                return <StatsTable jerseys={js} statsMap={st} roster={mr} />;
              })()}
            </div>
          </div>
        );
      ...
      {/* ═══ BENCH PICKER (sub modal) ═══ */}
      {subBenchOpen && (
        <div style={S.overlay}>
          <div style={{ ...S.card, width: "100%", maxWidth: 320, border: "1px solid #ca8a0433", animation: "fadeIn 0.2s ease" }}>
            <div style={{ textAlign: "center", marginBottom: 12 }}>
              <div style={{ fontSize: 26, marginBottom: 4 }}>🔄</div>
              <div style={{ fontSize: 15, fontWeight: 900 }}>Chi entra?</div>
            ...
            {!benchPlayers.length
              ? <div style={{ textAlign: "center", color: "#4d7fa8", fontSize: 13, padding: 16 }}>Nessuna in panchina</div>
              : <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 10 }}>
                {benchPlayers.map(p => (
                  <button key={p.jersey} onClick={() => handleSubPickBench(p.jersey)} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 14px", background: "#060e1a", border: "1px solid #1e3a5f", borderRadius: 12, cursor: "pointer" }}>
                    ...
                    <div style={{ flex: 1, textAlign: "left" }}>
                      ...
                      <div style={{ fontSize: 10, color: "#4d7fa8" }}>{p.role}</div>
                    </div>
                    <div style={{ fontSize: 16, color: "#1e3a5f" }}>›</div>
                  </button>
                ))}
            ...
            {subErr2 && <div style={{ color: "#ef4444", fontSize: 11, marginBottom: 8, textAlign: "center" }}>{subErr2}</div>}
            <button onClick={cancelSub} style={{ width: "100%", padding: 10, borderRadius: 11, background: "#0d1f35", border: "1px solid #1e3a5f", color: "#4d7fa8", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>Annulla</button>
          </div>
        </div>
      )}
      ...
      {showStats && (() => {
        ...
        const activeKey = statsTab;
        let statsData = {}, jerseys = [];
        if (activeKey === "current" || activeKey === String(setN)) { statsData = computeStats(setN); jerseys = [...rot, ...Object.keys(statsData).filter(j => !rot.includes(j))].filter(j => j); }
        else if (activeKey === "all") { statsData = computeStats("all"); jerseys = Object.keys(statsData).sort((a, b) => parseInt(a) - parseInt(b)); }
        else { statsData = computeStats(parseInt(activeKey)); jerseys = Object.keys(statsData).sort((a, b) => parseInt(a) - parseInt(b)); }
        return (
          <div style={{ position: "fixed", inset: 0, background: "#000000cc", zIndex: 200, display: "flex", flexDirection: "column", animation: "slideUp 0.3s ease" }}>
            <div style={{ flex: 1, overflowY: "auto", padding: 14 }}>
              <div style={{ maxWidth: 420, margin: "0 auto" }}>
                <div style={{ textAlign: "center", marginBottom: 10 }}>
                  <div style={{ fontSize: 13, fontWeight: 900, color: "#60a5fa" }}>📊 STATISTICHE — {teamALabel.toUpperCase()}</div>
                </div>
                <div style={{ display: "flex", gap: 5, marginBottom: 12, overflowX: "auto", paddingBottom: 3 }}>
                  {tabs.map(t => <button key={t.key} onClick={() => setStatsTab(t.key)} style={{ padding: "5px 11px", borderRadius: 20, fontWeight: 700, fontSize: 10, cursor: "pointer", whiteSpace: "nowrap", border: `1px solid ${activeKey === t.key ? "#3b82f6" : "#1e3a5f"}`, flexShrink: 0, background: activeKey === t.key ? "#1d4ed8" : "#0d1f35", color: activeKey === t.key ? "#fff" : "#4d7fa8" }}>{t.label}</button>)}
                </div>
                <StatsTable jerseys={jerseys} statsMap={statsData} roster={activeRoster} />
              </div>
            </div>
            <div style={{ padding: "10px 14px", background: "#0a1929", borderTop: "1px solid #1e3a5f", flexShrink: 0 }}>
      ...
      {saveTeamModal && (
        <div style={S.overlay}>
          <div style={{ ...S.card, width: "100%", maxWidth: 300, border: "1px solid #7c3aed44", animation: "fadeIn 0.2s ease" }}>
            <div style={{ textAlign: "center", marginBottom: 12 }}>
              <div style={{ fontSize: 26, marginBottom: 4 }}>💾</div>
              <div style={{ fontSize: 15, fontWeight: 900 }}>Salva distinta</div>
            </div>
            {currentTeam && <button onClick={handleSaveTeamOverwrite} style={{ ...S.btn("linear-gradient(135deg,#1d4ed8,#1e40af)"), marginBottom: 8, fontSize: 13 }}>Sovrascrive "{currentTeam.name}"</button>}
            {currentTeam && <div style={{ textAlign: "center", fontSize: 11, color: "#4d7fa8", margin: "4px 0" }}>— oppure —</div>}
            <div style={{ ...S.label, marginBottom: 5 }}>Salva come nuova squadra</div>
            <input type="text" value={newTeamName} onChange={e => setNewTeamName(e.target.value)} onKeyDown={e => e.key === "Enter" && handleSaveTeamNew()} placeholder="Nome squadra (es. Under 15)" style={{ ...S.inp, marginBottom: 8, textAlign: "center" }} />
            <button onClick={handleSaveTeamNew} disabled={!newTeamName.trim()} style={{ ...S.btn("linear-gradient(135deg,#16a34a,#15803d)"), opacity: newTeamName.trim() ? 1 : 0.4, marginBottom: 8 }}>Salva come nuova</button>
            <button onClick={() => setSaveTeamModal(false)} style={{ width: "100%", padding: 9, background: "none", border: "none", color: "#2d5a7a", cursor: "pointer", fontSize: 12 }}>Annulla</button>
          </div>
        </div>
      )}
    </div>
